...
The transaction_date column was converted into datetime format so that time-based analysis could be done properly. From this column, I also created a new month feature, which helped in analyzing monthly sales trends.The dataset was already cleaned, so no major missing value handling was required. After these steps, the data was ready for further analysis.
